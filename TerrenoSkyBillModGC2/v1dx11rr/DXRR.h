...
public:	
    ...
    DXRR(HWND hWnd, int Ancho, int Alto)
	{
		...
		arriaba = 0;
		billCargaFuego();
		camara = new Camara(D3DXVECTOR3(7,80,-9), D3DXVECTOR3(-6,5,42), D3DXVECTOR3(0,1,0), Ancho, Alto);
		...
		lamp = new ModeloRR(d3dDevice, d3dContext, "Assets/Lantern/pato.obj", L"Assets/Lantern/patoC.png", L"Assets/Lantern/patoN.png", 0, 0);
	...
	void Render(void)
	{
		...
		camara->UpdateCam(vel, arriaba, izqder, forward);
		lamp->Update(camara->posCam.x +5, camara->posCam.z + 5);
		camara->hdveo.x = lamp->getPosX() + 3 * sin(10);
		camara->hdveo.z = lamp->getPosZ() + 3 * cos(10);
		lamp->Update2(camara->vista, camara->proyeccion);
		...
		lamp->Draw(camara->vista, camara->proyeccion, terreno->Superficie(100, 20), camara->posCam, 10.0f, 0, 'A', 1);
