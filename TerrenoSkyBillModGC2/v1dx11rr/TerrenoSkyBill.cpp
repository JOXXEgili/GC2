...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(message)
    {
        ...
        case WM_MOUSEMOVE: {
            ...
            float magX = dxrr->camara->hdveo.x - dxrr->camara->posCam.x;
            float magZ = dxrr->camara->hdveo.z - dxrr->camara->posCam.z;
            wstring pos = L"X:" + to_wstring(dxrr->camara->posCam.x) + L" Y:" + to_wstring(dxrr->camara->posCam.y) + L" Z:" + to_wstring(dxrr->camara->posCam.z) + L"\n"
                L"VX:" + to_wstring(dxrr->camara->hdveo.x) + L" VY:" + to_wstring(dxrr->camara->hdveo.y) + L" VZ:" + to_wstring(dxrr->camara->hdveo.z);
            if (keyboardData[DIK_F1] & 0x80) {
                MessageBox(hWnd, pos.c_str(), L"POSICIÓN", MB_OK);
                break;  
            }
