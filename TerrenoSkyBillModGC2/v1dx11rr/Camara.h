...
public:
	...
	D3DXMATRIX UpdateCam(float vel, float arriaba, float izqder)
	{
		...
		posCam.x = 10 + 3 * sin(15);
		posCam.z = 10 + 3 * cos(15);
	...
	*/
	D3DXMATRIX UpdateCam(float vel, float arriaba, float izqder, bool forward)
	{
		D3DXVECTOR4 tempo;
		D3DXQUATERNION quatern; //quaternion temporal para la camara
		D3DXMATRIX giraUp, giraRight; //matrices temporales para los giros
		...
		if (forward)
		{
			posCam += refFront * vel / 10.0f;
			hdveo = (posCam + refFront);
		}
		else if (!forward)
		{
			posCam += refRight * vel / 10.0f;
			hdveo = (posCam + refFront);
		}
